...
export default function About() {
  ...
  const animSp = useTransform(sp, [0, 0.70], [0, 1])
  ...
  const sceneOpacity = useTransform(animSp, [0.28, 0.38], [0, 1])
  const sceneY = useTransform(animSp, [0.28, 0.38], [55, 0])
  const sceneScale = useTransform(animSp, [0.28, 0.38], [0.96, 1])

  // Deck placeholder
  const deckOpacity = useTransform(animSp, [0.34, 0.40, 0.50], [0, 0.45, 0])
  const deckY = useTransform(animSp, [0.34, 0.50], [120, 0])
  const deckScale = useTransform(animSp, [0.34, 0.50], [0.92, 1])

  // 6 cards — 2 rows × 3 columns
  const c0Op  = useTransform(animSp, [0.38, 0.46], [0, 1])
  const c0X   = useTransform(animSp, [0.38, 0.55], [0, -368])
  const c0Y   = useTransform(animSp, [0.38, 0.55], [60, -82])
  const c0R   = useTransform(animSp, [0.38, 0.55], [0, -10])
  const c0Sc  = useTransform(animSp, [0.38, 0.55], [0.82, 1])

  const c1Op  = useTransform(animSp, [0.47, 0.55], [0, 1])
  const c1X   = useTransform(animSp, [0.47, 0.64], [0, 0])
  const c1Y   = useTransform(animSp, [0.47, 0.64], [60, -86])
  const c1R   = useTransform(animSp, [0.47, 0.64], [0, 2])
  const c1Sc  = useTransform(animSp, [0.47, 0.64], [0.82, 1])

  const c2Op  = useTransform(animSp, [0.56, 0.64], [0, 1])
  const c2X   = useTransform(animSp, [0.56, 0.73], [0, 368])
  const c2Y   = useTransform(animSp, [0.56, 0.73], [60, -80])
  const c2R   = useTransform(animSp, [0.56, 0.73], [0, 9])
  const c2Sc  = useTransform(animSp, [0.56, 0.73], [0.82, 1])

  const c3Op  = useTransform(animSp, [0.70, 0.78], [0, 1])
  const c3X   = useTransform(animSp, [0.70, 0.87], [0, -368])
  const c3Y   = useTransform(animSp, [0.70, 0.87], [60, 318])
  const c3R   = useTransform(animSp, [0.70, 0.87], [0, 8])
  const c3Sc  = useTransform(animSp, [0.70, 0.87], [0.82, 1])

  const c4Op  = useTransform(animSp, [0.79, 0.87], [0, 1])
  const c4X   = useTransform(animSp, [0.79, 0.96], [0, 0])
  const c4Y   = useTransform(animSp, [0.79, 0.96], [60, 322])
  const c4R   = useTransform(animSp, [0.79, 0.96], [0, -4])
  const c4Sc  = useTransform(animSp, [0.79, 0.96], [0.82, 1])

  const c5Op  = useTransform(animSp, [0.88, 0.96], [0, 1])
  const c5X   = useTransform(animSp, [0.88, 1.00], [0, 368])
  const c5Y   = useTransform(animSp, [0.88, 1.00], [60, 320])
  const c5R   = useTransform(animSp, [0.88, 1.00], [0, -11])
  const c5Sc  = useTransform(animSp, [0.88, 1.00], [0.82, 1])
